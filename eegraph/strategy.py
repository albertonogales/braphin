...
# Heavy EEG-only dependencies imported lazily inside the estimators that use them:
#   antropy → Spectral_entropy_Estimator.single_channel_conn
#   pandas  → Shannon_entropy_Estimator.single_channel_conn
...
class Spectral_entropy_Estimator(Connectivity_single_channel_With_Bands):
    ...
    def single_channel_conn(self, data, sample_rate):
        import antropy as ant  # noqa: PLC0415  (lazy: EEG spectral entropy only)
        nperseg = len(data)
...
class Shannon_entropy_Estimator(Connectivity_single_channel_No_Bands):
    ...
    def single_channel_conn(self, data, sample_rate):
        import pandas as pd  # noqa: PLC0415  (lazy: EEG Shannon entropy only)
