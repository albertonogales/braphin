...
# Heavy EEG-only dependencies are imported lazily so that
# `from eegraph import Graph` succeeds with the fMRI-only install
# (numpy, scipy, nibabel, networkx) without requiring the full EEG stack.
# Each is imported inside the function/method that actually uses it.
#   pandas  → re_scaling, set_montage, input_bands (EEG only)
#   scot    → calculate_dtf (EEG DTF only)
#   plotly  → draw_graph, get_edge_trace, get_node_trace (visualisation only)
...
def re_scaling(raw_data):
    import pandas as pd  # lazy: EEG-only path
    # Bug fix: df.sub() returns a new DataFrame; must be reassigned.
    # Previous version discarded the result and returned unscaled data.
...
def calculate_dtf(data_intervals, steps, channels, sample_rate, bands, flag):
    import scot  # lazy: DTF-only path; note scot 0.2.1 is broken with scipy >= 1.x
    num_bands = sum(bands)
...
def draw_graph(G):
    ...
    # convert to plotly graph (lazy import — visualisation only)
